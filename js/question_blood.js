document.addEventListener('DOMContentLoaded', function() {
    const backgroundImage = document.getElementById('bloodimg');
    ...
    document.getElementById('question-form').addEventListener('submit', function(event) {
        ...
        const selectedScreams = document.querySelector('input[name="blood"]:checked').value;
        if (selectedScreams === 'yes') {
            volume = Math.min(volume + 0.1, 1);
            window.location.href = `question_screams.html?screams=${encodeURIComponent(selectedScreams)}&volume=${volume}`;
        } else {
